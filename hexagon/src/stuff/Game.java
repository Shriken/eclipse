...
public class Game implements KeyListener, MouseListener {
	...
	public Game() {
		...
		characterCreationMenu = Menu.read("characterCreation");
	}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (curPerspective == CHARACTER_CREATION) {
			if (e.getKeyCode() == KeyEvent.VK_J)
				characterCreationMenu.scrollDown();
			
			if (e.getKeyCode() == KeyEvent.VK_K)
				characterCreationMenu.scrollUp();
	...
	public ArrayList<Unit> loadUnits() {
		...
		File[] files = new File("res/units").listFiles();
		for (File f : files)
			units.add(new Unit(f.getPath()));
